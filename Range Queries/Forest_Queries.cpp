...
struct prefix_sum_2d {
	...
	void init (int m0, int n0) {
		m = m0; n = n0;
		ps = vector <vector <int>> (m, vector <int> (n));
	}
	void add_val (int i, int j, int v) {
		ps[i][j] = v;
	}
	void run() {
		for (int i = 0; i < m; i ++) for (int j = 1; j < n; j ++) ps[i][j] += ps[i][j - 1];
		for (int j = 0; j < n; j ++) for (int i = 1; i < m; i ++) ps[i][j] += ps[i - 1][j];
	}
	int query (int x1, int y1, int x2, int y2) {
		return ps[x2][y2] - ((y1 > 0) ? ps[x2][y1 - 1] : 0) - ((x1 > 0) ? ps[x1 - 1][y2] : 0) + ((x1 > 0 && y1 > 0) ? ps[x1 - 1][y1 - 1] : 0);
	}
};
int main() {
	int n, q;
	...
	for (int i = 0; i < n; i ++) {
		for (int j = 0; j < n; j ++) {
			char c;
			cin >> c;
			if (c == '*') z.add_val(i, j, 1);
			else z.add_val(i, j, 0);
		}
	}
	z.run();
	while (q --) {
