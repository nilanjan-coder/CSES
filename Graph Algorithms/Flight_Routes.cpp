...
struct graph_k_dijkstra {
	...
	void run() {
		...
		while (! pq.empty()) {
			...
			pq.pop();
			if (vis[u.second] == k) continue;
			vis[u.second] ++;
			for (pair <ll, int> v: g[u.second]) {
				if (u.first + v.first < dist[v.second][k - 1]) {
					dist[v.second][k - 1] = u.first + v.first;
					pq.push({dist[v.second][k - 1], v.second});
					sort(dist[v.second].begin(), dist[v.second].end());
				}
			}
		}
	}
