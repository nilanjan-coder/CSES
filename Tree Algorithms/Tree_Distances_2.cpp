...
using ll = long long;
struct tree_reroot_sum_dist {
	int n;
	vector <vector <int>> g;
	vector <ll> sz, in, out, dist;
	// in[u] = sum of dist of nodes in subtree rooted at u from u
	// out[u] = sum of dist of nodes in the tree from u when it is rooted at u but the subtree rooted at u is deleted
	void init (int s) {
		n = s;
		g.resize(n);
		in.assign(n, 0);
