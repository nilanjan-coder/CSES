...
using ll = long long;
struct tree_merge_count_paths_of_length_at_most_k {
	int n, k;
	ll ans;
	vector <vector <int>> g;
	vector <deque <ll>> cnt;
	void init (int s, int s2) {
		n = s;
		k = s2;
		ans = 0;
		g.resize(n);
	...
	void dfs (int u, int p) {
		for (int v: g[u]) {
			if (v - p) {
				dfs(v, u);
				cnt[v].push_front(0);
				for (int i = max(0, k - (int) cnt[v].size() + 1); i <= min(k, (int) cnt[u].size() - 1); i ++) {
					ans += cnt[u][i] * cnt[v][k - i];
				}
				if ((int) cnt[u].size() < (int) cnt[v].size()) {
					swap(cnt[u], cnt[v]);
				}
				for (int i = 0; i < (int) cnt[v].size(); i ++) {
					cnt[u][i] += cnt[v][i];
				}
			}
		}
...
int main() {
	int n, k1, k2;
	cin >> n >> k1 >> k2;
	tree_merge_count_paths_of_length_at_most_k t1, t2;
	t1.init(n, k1 - 1);
	t2.init(n, k2);
	for (int i = 1; i <= n - 1; i ++) {
		int a, b;
